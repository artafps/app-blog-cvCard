...
const ProjectS = () => {
    ...
    const handleChangeTITLEPROJECT = async (events) => {
        ...
        if (Data === null) {
            ...
            const newData = JSON.parse(Data)
            newData.Project.Title = String(events)
            localStorage.setItem('DATAGITBACK', JSON.stringify(newData))
            setTITLE(events)
        }
        Data = localStorage.getItem('DATAGITBACK')
    ...
    const handleChangePROJECTITEMS = async () => {
        ...
        try {
            var events = `https://api.github.com/repos/${OwnerName}/${RepoName}/contents/project/jpg/PROJECT-f-${File.name}`
            var icon = `https://raw.githubusercontent.com/${OwnerName}/${RepoName}/main/project/jpg/PROJECT-f-${File.name}`
    ...
    const handleDeleteITEM_PROJECT = async (id) => {
        ...
        var d = localStorage.getItem('DATAGITBACK')
        if (d === null) {
            const newDataSocial = DATAGITARRY.Project.Arr.filter(item => item.ImageURI !== id)
            DATAGITARRY.Project.Arr = newDataSocial
            localStorage.setItem('DATAGITBACK', JSON.stringify(DATAGITARRY))
            HANDLESAVE()
        } else {
            let Data = JSON.parse(d)
    ...
    return (<Dashboard title='Project' disc='Edit the project section'>
